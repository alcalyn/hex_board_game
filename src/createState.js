...
import { WHO_RED, WHO_BLUE, WHO_NONE, sign, parseMove, convertMovesHistoryToCells } from './utils.js';

/**
 * @param {Number} level From 1 to 10 (Defaults to 10)
 */
export function State(level) {
    let currentMove = WHO_RED;
    let theComputer = WHO_RED;
    let theComputerLevel = 10;
     ...
     */
    this.resetBoard = cells => {
        init();

        if (!Array.isArray(cells) || !Array.isArray(cells[0]) || !Array.isArray(cells[1])) {
            throw new Error("resetBoard must be called with red and blue cells. Ex: resetBoard([['d4', 'f5'], ['a6']])");
        }

        // const theCol = WHO_RED === who ? -1 : 1;

        for (const cell of cells[0]) {
            const [ii, jj] = parseMove(cell);
            Fld[jj][ii] = -1;
            ++MoveCount;
        }
     ...
     */
    this.replayMovesHistory = movesHistory => {
        ...
        this.resetBoard(convertMovesHistoryToCells(movesHistory));
    };
    ...
    this.setLevel = level => {
        ...
        theComputerLevel = level;
    };

    this.setLevel(level ?? theComputerLevel);

    this.getBestMove = (who, theLevel) => {
        if (!theLevel) {
            theLevel = theComputerLevel ?? 10;
        }

        this.setLevel(theLevel ?? theComputerLevel);
